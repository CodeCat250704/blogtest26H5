...
(function() {
    ...
    const navigateTo = (module, targetTitle) => {
        ...
        if (module === 'categories' && targetTitle) {
            window.location.href = `/modular-pad/categories/index.html?target=${encodeURIComponent(targetTitle)}`;
        }
    };
        ...
        try {
            ...
            if (posts.length > 0) {
                ...
                historyList.innerHTML = sortedPosts.map(post => `
                    <li class="history-link" data-title="${post.title}" style="cursor:pointer; transition:0.2s; color:var(--text-muted); list-style:none; padding:6px 0; border-bottom:1px solid rgba(255,255,255,0.03);">
                        · ${post.title}
                    </li>
                `).join('');
                ...
                <div class="player-panel">
                    ...
                    <div class="player-controls" style="flex-shrink: 0;">
                        ...
                        <div style="display: flex; gap: 10px; align-items: center;">
                            ...
                            <button class="ctrl-btn" id="playPauseBtn" style="font-size: 16px;"><i class="fa-solid fa-circle-play"></i></button>
                        ...
                        <div class="volume-container" style="display: flex; align-items: center; gap: 4px;">
                            <i class="fa-solid fa-volume-high" style="color: var(--text-muted); font-size: 13px;"></i>
                            <input type="range" class="vol-slider" id="musicVolumeSlider" min="0" max="1" step="0.01" value="0.5">
                        </div>
                    </div>

                    <input type="text" class="music-search-box" id="musicSearchInput" placeholder="搜索歌曲..." style="flex-shrink: 0;">
                    
                    <div class="music-item-container">
                        ${songs.map((name, index) => `
                            <div class="music-item" data-index="${index}">
                                <i class="fa-regular fa-circle-play" style="margin-right: 6px; font-size: 12px; color: var(--text-muted);"></i>
